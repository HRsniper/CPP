...
int main(int argc, const char *argv[]) {
    ...
    /* buffer */
    unsigned int buffer;
    GlCall_F_DebugBreak(glGenBuffers(1, &buffer));
    GlCall_F_DebugBreak(glBindBuffer(GL_ARRAY_BUFFER, buffer));
    GlCall_F_DebugBreak(glBufferData(GL_ARRAY_BUFFER, 6 * 2 * sizeof(float), positions, GL_STATIC_DRAW));
    ...
    /*index buffer object*/
    unsigned int indexB;
    GlCall_F_DebugBreak(glGenBuffers(1, &indexB));
    GlCall_F_DebugBreak(glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, indexB));
    GlCall_F_DebugBreak(
